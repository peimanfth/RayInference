...
class ModelManager:
    def __init__(self, model_name: str, local_path=None, batch_size=None):
        if local_path:  # Path to local model directory is provided
            ...
            if model_name == "llama-2-7b-chat-hf":
                ...
                if batch_size:
                    self.pipeline = pipeline('text-generation', model=local_path, torch_dtype=torch.float32, device=0, batch_size=batch_size)
                    self.pipeline.tokenizer.pad_token_id = self.pipeline.model.config.eos_token_id
                else:
                    self.pipeline = pipeline('text-generation', model=local_path, torch_dtype=torch.float32, device=0)
        else:
