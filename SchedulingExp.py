...
def process_batch(batch, distributed_model, batch_size, model_name, batch_number, scheduling_delay):
    ...
    batch_info = {
        ...
        'scheduled_delay': scheduling_delay,  # Time taken to select the actor
...
def HashBasedSelection(batch,actors):
    ...
    end_time = time.time()
    duration = (end_time - start_time) * 1000
    return actors[index], duration



def run_experiment(data, batch_sizes, num_actors, local_model_paths, algorithm):
    all_results = []
    for batch_size in batch_sizes:
        ...
        for model_name, actors in model_actors.items():
            ...
            for i, batch in enumerate(model_data.iter_batches(batch_size=batch_size, batch_format='pandas')):
                if algorithm == "RoundRobinSelection":
                    actor, delay = RoundRobinSelection(batch_number,actors)
                elif algorithm == "RandomSelection":
                    actor, delay = RandomSelection(actors)
                elif algorithm == "HashBasedSelection":
                    actor, delay = HashBasedSelection(batch,actors)
                ...
                futures.append(process_batch.remote(batch, actor, batch_size, model_name, batch_number, delay))
...
def main():
    ...
    max_actors = 3
    algorithms = ["RoundRobinSelection", "RandomSelection", "HashBasedSelection"]

    for algorithm in algorithms:


        batch_sizes = [2**i for i in range(2, 5)]  # Batch sizes 16, 32, 64, 128, 256

        experiment_results = run_experiment(data, batch_sizes, max_actors, local_model_paths, algorithm)

        current_time = datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
        filename = f'logs/multiActor/algo/experiment_results_{algorithm}_{current_time}.json'
        os.makedirs(os.path.dirname(filename), exist_ok=True)

        with open(filename, 'w') as f:
            json.dump({data_path: experiment_results}, f, indent=4)

        print(f'Experiment {algorithm} actors completed and results saved."')
